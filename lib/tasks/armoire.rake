...
namespace :armoire do
  ...
  task create: :environment do
    ...
    armoires = XIVAPI_CLIENT.content(name: 'Cabinet', columns: ARMOIRE_COLUMNS, limit: 1000).map do |armoire|
      ...
      data = { id: armoire.id, category_id: armoire.category_target_id, order: armoire.order }

      %w(en de fr ja).each do |locale|
        data["name_#{locale}"] = sanitize_name(armoire.item["name_#{locale}"])
      end

      download_image(armoire.id, armoire.item.icon, 'armoires')

      if existing = Armoire.find_by(id: armoire.id)
